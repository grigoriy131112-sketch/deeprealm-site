...
// The site runs in two places: the Node server, which answers /api, and static
// hosting such as GitHub Pages, which answers nothing. Both serve the same data,
// so every read tries the API first and falls back to the JSON shipped beside
// the page. That keeps one frontend working in both setups.
async function readJson(apiPath, staticPath) {
  try {
    const res = await fetch(apiPath);
    if (res.ok) return await res.json();
  } catch { /* static hosting: no server to answer */ }
